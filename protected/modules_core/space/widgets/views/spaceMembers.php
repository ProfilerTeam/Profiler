...
<div class="panel panel-default members" id="space-members-panel">
    ...
    <div class="panel-heading"><?php echo Yii::t('SpaceModule.widgets_views_spaceMembers', '<strong>Group</strong> members'); ?></div>
    <div class="panel-body">
        ...
        if ($space->isMember(Yii::app()->user->id)) {
            if (!$space->isSpaceOwner(Yii::app()->user->id)) {
                print CHtml::link('<i class="fa fa-sign-out"></i> '. Yii::t('SpaceModule.widgets_views_spaceMembers', "Leave group"), $this->createUrl('//space/space/revokeMembership', array('sguid' => $space->guid)), array('class' => 'btn btn-danger'));
            }
        } else {
