        ...
        jQuery.getJSON(options.searchUrl.replace('-keywordPlaceholder-', string), function(json) {
            ...
            if (json.length > 0) {
                ...
                for (var i = 0; i < json.length; i++) {
                    // build <li> entry
                    var str = '<li><a tabindex="-1" href="javascript:addSpaceTag(\'' + json[i].guid + '\', \'' + json[i].image + '\', \'' + addslashes(htmlDecode(json[i].title)) + '\');"><img class="img-rounded" src="' + json[i].image + '" alt=""/> ' + json[i].title + '</a></li>';
